      ...
      for (var stationJson in region["Stations"]) {
        ...
        if (stationJson["Components"] != null) {
          for (var componentJson in stationJson["Components"]) {
            var component =
                Component(componentJson["Code"], componentJson["Int"], componentJson["Ix"]);
...
Map parseLegend(jsonResponse) {
  ...
  for (int i = 0; i < jsonResponse["Legend"].length; i++) {
    ...
    var legendsItem = LegendItem(
        legendJson["Ix"], Color.hexToColor("#" + legendJson["Color"]), legendJson["Description"]);
...
Map parseComponents(jsonResponse) {
  ...
  for (var componentJson in jsonResponse["Components"]) {
    components[componentJson["Code"]] =
        ComponentLegendItem(componentJson["Code"], componentJson["Name"], componentJson["Unit"]);
  }
  return components;
}
